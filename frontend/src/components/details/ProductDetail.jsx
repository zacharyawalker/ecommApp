...
//   // Fetch libraries for the library dialog
...
//     fetchMockups("My Mockups"); // Default to "My Mockups"
//   };
...
//   // Toggle between "My Mockups" and "All Mockups" in mockups dialog
...
//     console.log("Selected mockup set with id:", mockup.id); // Debug line
//   };
...
//   // Confirm addition of selected mockup or library to product
//   const handleAddConfirm = async () => {
//     try {
//       setLoadingAdd(true); // Show loading modal
...
//       window.location.reload(); // Refresh to show the added items in the carousel
//     } catch (error) {
//       console.error("Error adding to product:", error);
//     } finally {
//       setLoadingAdd(false); // Hide loading modal after request completes
//     }
//   };
...
//             <Box
//               key={image.id}
//               onClick={() => handleThumbnailClick(image.image_path)}
//               sx={{
//                 cursor: 'pointer',
...
//                 <Box
//                   onClick={() => handleDialogThumbnailClick(mockup)}
//                   sx={{
//                     cursor: 'pointer',
//                     padding: '5px',
//                     borderRadius: 2,
...
//     }
//   };
//   const handleRemoveMockup = async (mockupId) => {
//     try {
//       await axios.patch(`http://localhost:8000/api/v1/products/${productId}/remove_mockups/`, {
//         mockups: [mockupId],
//       });
//       // Refresh product data to reflect the removed mockup
//       const updatedProduct = await axios.get(`http://localhost:8000/api/v1/products/${productId}`);
...
//     gap: 1,
//     flexWrap: 'wrap',
//     justifyContent: product.product_images.length > 0 ? 'flex-start' : 'center',
//     alignItems: 'flex-start',
//     minHeight: '50px' // Ensures some height even if empty
//   }}
// >
...
//         <IconButton
//           color="error"
//           onClick={() => handleRemoveMockup(image.mockup)}
//           sx={{
//             position: 'absolute',
//             bottom: '5px',
//             left: '50%',
...
//           </IconButton>
//         </Box>
//         {mockupLibraries.length > 0 ? (
//           mockupLibraries.map((library) => (
//             <Box key={library.id} sx={{ border: '1px solid #ddd', borderRadius: 2, p: 2, mb: 2 }}>
//               <Typography variant="subtitle1" fontWeight="bold" gutterBottom>{library.title}</Typography>
//               <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', justifyContent: 'flex-start', alignItems: 'flex-start' }}>
//                 {library.mockups.map((image) => (
//                   <Box key={image.id} onClick={() => handleThumbnailClick(image.image_path)} sx={{
//                       cursor: 'pointer',
//                       border: mainImage === image.image_path ? '2px solid #1976d2' : '2px solid transparent',
//                       borderRadius: 2,
...
//                     }}
//                   >
//                     <CardMedia component="img" image={image.image_path} alt={`Library Mockup ${image.id}`} sx={{ width: '100%', height: 'auto' }} />
//                   </Box>
//                 ))}
//               </Box>
//             </Box>
...
//         ) : (
//           <Typography variant="body2" color="textSecondary">
//             No library mockups available. Use the "+" icon to add mockup libraries.
//           </Typography>
//         )}
//       </Box>
...
//         <DialogTitle>Add a Mockup</DialogTitle>
//         <DialogContent>
//           <FormControlLabel control={<Switch checked={showMyMockups} onChange={handleToggleChange} />} label={showMyMockups ? 'My Mockups' : 'All Mockups'} />
//           <Grid container spacing={2}>
//             {mockups.map((mockup) => (
//               <Grid item xs={3} key={mockup.id}>
//                 <Box onClick={() => handleDialogThumbnailClick(mockup)} sx={{
